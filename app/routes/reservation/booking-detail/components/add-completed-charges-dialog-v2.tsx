...
import { useServiceList } from "~/routes/services/container/services/query.hooks";
import type { MenuListItemDto } from "~/services/api/menu/dto";
import type { ServiceListItemDto } from "~/services/api/services/dto";
...
export default function AddCompletedChargesDialog({
  ...
  const [selectedServiceItems, setSelectedServiceItems] = useState<
    Map<string, { item: ServiceListItemDto; quantity: number }>
  >(new Map());
  ...
  const { data: serviceItems = [], isPending: isLoadingServices } =
    useServiceList({});
  ...
  const handleToggleServiceItem = (item: ServiceListItemDto) => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Thêm các món ăn/dịch vụ đã sử dụng nhưng chưa order trước
          </DialogDescription>
        </DialogHeader>
        ...
        <Alert className="bg-blue-50 dark:bg-blue-950/20 border-blue-200">
          ...
          <AlertDescription className="text-xs">
            Các item được thêm sẽ{" "}
            <strong>tự động được đánh dấu đã hoàn thành</strong> và xuất hiện
            trong pending charges để checkout.
          </AlertDescription>
        </Alert>

        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as "pos" | "service")} className="flex-1 flex flex-col overflow-hidden">
          ...
          <TabsContent value="pos" className="flex-1 overflow-hidden mt-4">
            <div className="grid grid-cols-12 gap-4 h-full">
              ...
              <div className="col-span-3 space-y-2 overflow-y-auto">
                ...
                {isLoadingCategories ? (
                  ...
                  <div className="space-y-1">
                    <Button
                      type="button"
                      variant={selectedCategoryCode === null ? "secondary" : "ghost"}
              ...
              {/* Menu Items */}
              <div className="col-span-6 space-y-2 flex flex-col overflow-hidden">
                <div className="space-y-2">
              ...
              <div className="col-span-3 space-y-2 flex flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto space-y-2">
                  {selectedPOSItems.size > 0 ? (
                    <>
                      {Array.from(selectedPOSItems.values()).map(({ item, quantity }) => (
                        <Card key={item.itemId} className="p-3 space-y-2">
                          ...
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <Counter
                              value={quantity}
                              onChange={(value) =>
                                handleUpdatePOSQuantity(item.itemId, value || 0)
                              }
                            />
                          </div>
          ...
          <TabsContent value="service" className="flex-1 overflow-hidden mt-4">
            <div className="grid grid-cols-12 gap-4 h-full">
              {/* Service Items List */}
              <div className="col-span-9 space-y-2 flex flex-col overflow-hidden">
                <div className="space-y-2">
                ...
                <div className="flex-1 overflow-y-auto space-y-2 p-2">
                  {isLoadingServices ? (
                    ...
                    filteredServiceItems.map((item) => {
                      const isSelected = selectedServiceItems.has(item.itemId);
                      const selectedData = selectedServiceItems.get(item.itemId);
              ...
              <div className="col-span-3 space-y-2 flex flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto space-y-2">
                  {selectedServiceItems.size > 0 ? (
                    <>
                      {Array.from(selectedServiceItems.values()).map(({ item, quantity }) => (
                        <Card key={item.itemId} className="p-3 space-y-2">
                          <div>
                            <p className="font-medium text-xs">{item.name}</p>
                            <p className="text-xs text-muted-foreground">
                          ...
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <Counter
                              value={quantity}
                              onChange={(value) =>
                                handleUpdateServiceQuantity(item.itemId, value || 0)
                              }
                            />
                          </div>
                          <div className="pt-2 border-t">
          ...
          <Card className="p-4 bg-muted/50 border-primary">
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold">{totalPOSCount} món - {formatMoney(totalPOSAmount).vndFormatted}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Dịch vụ:</span>
                <span className="font-semibold">{totalServiceCount} dịch vụ - {formatMoney(totalServiceAmount).vndFormatted}</span>
              </div>
